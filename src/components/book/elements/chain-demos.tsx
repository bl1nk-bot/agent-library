"use client";

import { useState } from "react";
import { cn } from "@/lib/utils";
import {
  Play,
  RotateCcw,
  CheckCircle2,
  XCircle,
  AlertTriangle,
  ArrowRight,
  ArrowDown,
  FileText,
  Search,
  Edit3,
  Sparkles,
  Package,
} from "lucide-react";

type StepStatus = "pending" | "running" | "success" | "failed" | "invalid" | "retrying";

// ============================================
// Validation Demo
// ============================================

interface ValidationStep {
  id: string;
  name: string;
  status: StepStatus;
  output?: string;
  validationResult?: "valid" | "invalid";
  reason?: string;
}

export function ValidationDemo() {
  const [scenario, setScenario] = useState<"valid" | "invalid">("invalid");
  const [isRunning, setIsRunning] = useState(false);
  const [steps, setSteps] = useState<ValidationStep[]>([
    { id: "generate", name: "Generate Data", status: "pending" },
    { id: "validate", name: "Validate Output", status: "pending" },
    { id: "process", name: "Process Data", status: "pending" },
  ]);

  const delay = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

  const updateStep = (id: string, updates: Partial<ValidationStep>) => {
    setSteps((prev) => prev.map((s) => (s.id === id ? { ...s, ...updates } : s)));
  };

  const resetSteps = () => {
    setSteps([
      { id: "generate", name: "Generate Data", status: "pending" },
      { id: "validate", name: "Validate Output", status: "pending" },
      { id: "process", name: "Process Data", status: "pending" },
    ]);
  };

  const runDemo = async () => {
    setIsRunning(true);
    resetSteps();
    await delay(300);

    // Step 1: Generate data
    updateStep("generate", { status: "running" });
    await delay(800);

    if (scenario === "invalid") {
      // First attempt produces invalid data
      updateStep("generate", {
        status: "success",
        output: '{"name": "John", "age": "twenty-five"}',
      });

      await delay(400);
      updateStep("validate", { status: "running" });
      await delay(600);
      updateStep("validate", {
        status: "failed",
        validationResult: "invalid",
        reason: "age must be a number, got string",
      });

      await delay(600);
      // Retry with feedback
      updateStep("generate", {
        status: "retrying",
        output: "Retrying with validation feedback...",
      });
      await delay(1000);
      updateStep("generate", {
        status: "success",
        output: '{"name": "John", "age": 25}',
      });

      await delay(400);
      updateStep("validate", { status: "running", validationResult: undefined, reason: undefined });
      await delay(600);
      updateStep("validate", {
        status: "success",
        validationResult: "valid",
        reason: "All fields valid",
      });
    } else {
      // Valid scenario
      updateStep("generate", {
        status: "success",
        output: '{"name": "John", "age": 25}',
      });

      await delay(400);
      updateStep("validate", { status: "running" });
      await delay(600);
      updateStep("validate", {
        status: "success",
        validationResult: "valid",
        reason: "All fields valid",
      });
    }

    await delay(400);
    updateStep("process", { status: "running" });
    await delay(700);
    updateStep("process", { status: "success", output: "Data processed successfully" });

    setIsRunning(false);
  };

  return (
    <div className="my-6 overflow-hidden rounded-lg border">
      <div className="bg-muted/50 flex items-center justify-between border-b px-4 py-3">
        <span className="text-sm font-medium">Validation Between Steps</span>
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              setScenario("valid");
              resetSteps();
            }}
            className={cn(
              "rounded px-2 py-1 text-xs",
              scenario === "valid"
                ? "bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300"
                : "bg-muted"
            )}
          >
            Valid Data
          </button>
          <button
            onClick={() => {
              setScenario("invalid");
              resetSteps();
            }}
            className={cn(
              "rounded px-2 py-1 text-xs",
              scenario === "invalid"
                ? "bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-300"
                : "bg-muted"
            )}
          >
            Invalid → Retry
          </button>
          <button
            onClick={runDemo}
            disabled={isRunning}
            className="bg-primary text-primary-foreground flex items-center gap-1.5 rounded px-3 py-1.5 text-sm font-medium disabled:opacity-50"
          >
            <Play className="h-3.5 w-3.5" />
            Run
          </button>
        </div>
      </div>

      <div className="p-4">
        <div className="grid gap-4 md:grid-cols-3">
          {/* Generate Step */}
          <div
            className={cn(
              "rounded-lg border-2 p-4 transition-all",
              steps[0].status === "pending" && "border-muted-foreground/30 border-dashed",
              steps[0].status === "running" && "border-blue-400 bg-blue-50 dark:bg-blue-950/30",
              steps[0].status === "success" && "border-green-400 bg-green-50 dark:bg-green-950/30",
              steps[0].status === "retrying" && "border-amber-400 bg-amber-50 dark:bg-amber-950/30"
            )}
          >
            <div className="text-muted-foreground mb-2 text-xs tracking-wide uppercase">Step 1</div>
            <div className="mb-2 flex items-center gap-2">
              {steps[0].status === "pending" && (
                <div className="border-muted-foreground/30 h-4 w-4 rounded-full border-2" />
              )}
              {steps[0].status === "running" && (
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-blue-500 border-t-transparent" />
              )}
              {steps[0].status === "success" && <CheckCircle2 className="h-4 w-4 text-green-500" />}
              {steps[0].status === "retrying" && (
                <RotateCcw className="h-4 w-4 animate-spin text-amber-500" />
              )}
              <span className="text-sm font-medium">{steps[0].name}</span>
            </div>
            {steps[0].output && (
              <div className="bg-muted/50 mt-2 rounded p-2 font-mono text-xs break-all">
                {steps[0].output}
              </div>
            )}
          </div>

          {/* Validate Step */}
          <div
            className={cn(
              "rounded-lg border-2 p-4 transition-all",
              steps[1].status === "pending" && "border-muted-foreground/30 border-dashed",
              steps[1].status === "running" && "border-blue-400 bg-blue-50 dark:bg-blue-950/30",
              steps[1].status === "success" && "border-green-400 bg-green-50 dark:bg-green-950/30",
              steps[1].status === "failed" && "border-red-400 bg-red-50 dark:bg-red-950/30"
            )}
          >
            <div className="text-muted-foreground mb-2 text-xs tracking-wide uppercase">Step 2</div>
            <div className="mb-2 flex items-center gap-2">
              {steps[1].status === "pending" && (
                <div className="border-muted-foreground/30 h-4 w-4 rounded-full border-2" />
              )}
              {steps[1].status === "running" && (
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-blue-500 border-t-transparent" />
              )}
              {steps[1].status === "success" && <CheckCircle2 className="h-4 w-4 text-green-500" />}
              {steps[1].status === "failed" && <XCircle className="h-4 w-4 text-red-500" />}
              <span className="text-sm font-medium">{steps[1].name}</span>
            </div>
            {steps[1].validationResult === "invalid" && (
              <div className="mt-2 rounded bg-red-100 p-2 dark:bg-red-900/30">
                <p className="text-xs font-medium text-red-700 dark:text-red-300">✗ INVALID</p>
                <p className="mt-1 text-xs text-red-600 dark:text-red-400">{steps[1].reason}</p>
              </div>
            )}
            {steps[1].validationResult === "valid" && (
              <div className="mt-2 rounded bg-green-100 p-2 dark:bg-green-900/30">
                <p className="text-xs font-medium text-green-700 dark:text-green-300">✓ VALID</p>
                <p className="mt-1 text-xs text-green-600 dark:text-green-400">{steps[1].reason}</p>
              </div>
            )}
            {steps[1].status === "pending" && (
              <p className="text-muted-foreground text-xs">Checks output schema & types</p>
            )}
          </div>

          {/* Process Step */}
          <div
            className={cn(
              "rounded-lg border-2 p-4 transition-all",
              steps[2].status === "pending" && "border-muted-foreground/30 border-dashed",
              steps[2].status === "running" && "border-blue-400 bg-blue-50 dark:bg-blue-950/30",
              steps[2].status === "success" && "border-green-400 bg-green-50 dark:bg-green-950/30"
            )}
          >
            <div className="text-muted-foreground mb-2 text-xs tracking-wide uppercase">Step 3</div>
            <div className="mb-2 flex items-center gap-2">
              {steps[2].status === "pending" && (
                <div className="border-muted-foreground/30 h-4 w-4 rounded-full border-2" />
              )}
              {steps[2].status === "running" && (
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-blue-500 border-t-transparent" />
              )}
              {steps[2].status === "success" && <CheckCircle2 className="h-4 w-4 text-green-500" />}
              <span className="text-sm font-medium">{steps[2].name}</span>
            </div>
            {steps[2].output && <p className="text-muted-foreground text-xs">{steps[2].output}</p>}
            {steps[2].status === "pending" && (
              <p className="text-muted-foreground text-xs">Uses validated data</p>
            )}
          </div>
        </div>

        {/* Retry indicator */}
        {scenario === "invalid" && steps[0].status === "retrying" && (
          <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/30">
            <p className="text-xs text-amber-700 dark:text-amber-300">
              <span className="font-medium">↺ Retrying Step 1</span> — Validation failed,
              re-generating with feedback: "{steps[1].reason}"
            </p>
          </div>
        )}
      </div>
    </div>
  );
}

// ============================================
// Fallback Demo
// ============================================

interface FallbackStep {
  id: string;
  name: string;
  type: "primary" | "fallback";
  status: StepStatus;
  output?: string;
  confidence?: number;
}

export function FallbackDemo() {
  const [scenario, setScenario] = useState<"primary" | "fallback">("fallback");
  const [isRunning, setIsRunning] = useState(false);
  const [steps, setSteps] = useState<FallbackStep[]>([
    { id: "primary", name: "Complex Analysis", type: "primary", status: "pending" },
    { id: "fallback", name: "Simple Extraction", type: "fallback", status: "pending" },
    { id: "output", name: "Final Result", type: "primary", status: "pending" },
  ]);

  const delay = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

  const updateStep = (id: string, updates: Partial<FallbackStep>) => {
    setSteps((prev) => prev.map((s) => (s.id === id ? { ...s, ...updates } : s)));
  };

  const resetSteps = () => {
    setSteps([
      { id: "primary", name: "Complex Analysis", type: "primary", status: "pending" },
      { id: "fallback", name: "Simple Extraction", type: "fallback", status: "pending" },
      { id: "output", name: "Final Result", type: "primary", status: "pending" },
    ]);
  };

  const runDemo = async () => {
    setIsRunning(true);
    resetSteps();
    await delay(300);

    updateStep("primary", { status: "running" });
    await delay(1000);

    if (scenario === "fallback") {
      updateStep("primary", {
        status: "failed",
        output: "Low confidence (32%)",
        confidence: 32,
      });

      await delay(500);
      updateStep("fallback", { status: "running" });
      await delay(800);
      updateStep("fallback", {
        status: "success",
        output: "Extracted key entities",
        confidence: 95,
      });

      await delay(400);
      updateStep("output", { status: "running" });
      await delay(600);
      updateStep("output", {
        status: "success",
        output: "Result from fallback (partial data)",
      });
    } else {
      updateStep("primary", {
        status: "success",
        output: "Deep analysis complete",
        confidence: 94,
      });

      await delay(400);
      updateStep("output", { status: "running" });
      await delay(600);
      updateStep("output", {
        status: "success",
        output: "Result from primary (full analysis)",
      });
    }

    setIsRunning(false);
  };

  return (
    <div className="my-6 overflow-hidden rounded-lg border">
      <div className="bg-muted/50 flex items-center justify-between border-b px-4 py-3">
        <span className="text-sm font-medium">Fallback Chain Demo</span>
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              setScenario("primary");
              resetSteps();
            }}
            className={cn(
              "rounded px-2 py-1 text-xs",
              scenario === "primary"
                ? "bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300"
                : "bg-muted"
            )}
          >
            Primary Succeeds
          </button>
          <button
            onClick={() => {
              setScenario("fallback");
              resetSteps();
            }}
            className={cn(
              "rounded px-2 py-1 text-xs",
              scenario === "fallback"
                ? "bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300"
                : "bg-muted"
            )}
          >
            Use Fallback
          </button>
          <button
            onClick={runDemo}
            disabled={isRunning}
            className="bg-primary text-primary-foreground flex items-center gap-1.5 rounded px-3 py-1.5 text-sm font-medium disabled:opacity-50"
          >
            <Play className="h-3.5 w-3.5" />
            Run
          </button>
        </div>
      </div>

      <div className="p-4">
        <div className="grid gap-4 md:grid-cols-3">
          {/* Primary */}
          <div
            className={cn(
              "rounded-lg border-2 p-4 transition-all",
              steps[0].status === "pending" && "border-muted-foreground/30 border-dashed",
              steps[0].status === "running" && "border-blue-400 bg-blue-50 dark:bg-blue-950/30",
              steps[0].status === "success" && "border-green-400 bg-green-50 dark:bg-green-950/30",
              steps[0].status === "failed" && "border-red-400 bg-red-50 dark:bg-red-950/30"
            )}
          >
            <div className="text-muted-foreground mb-2 text-xs tracking-wide uppercase">
              Primary
            </div>
            <div className="mb-2 flex items-center gap-2">
              {steps[0].status === "running" && (
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-blue-500 border-t-transparent" />
              )}
              {steps[0].status === "success" && <CheckCircle2 className="h-4 w-4 text-green-500" />}
              {steps[0].status === "failed" && <XCircle className="h-4 w-4 text-red-500" />}
              {steps[0].status === "pending" && (
                <div className="border-muted-foreground/30 h-4 w-4 rounded-full border-2" />
              )}
              <span className="text-sm font-medium">{steps[0].name}</span>
            </div>
            {steps[0].output && <p className="text-muted-foreground text-xs">{steps[0].output}</p>}
            {steps[0].confidence && (
              <div className="mt-2">
                <div className="bg-muted h-1.5 overflow-hidden rounded-full">
                  <div
                    className={cn(
                      "h-full transition-all",
                      steps[0].confidence > 70 ? "bg-green-500" : "bg-red-500"
                    )}
                    style={{ width: `${steps[0].confidence}%` }}
                  />
                </div>
                <p className="text-muted-foreground mt-1 text-xs">
                  Confidence: {steps[0].confidence}%
                </p>
              </div>
            )}
          </div>

          {/* Fallback */}
          <div
            className={cn(
              "rounded-lg border-2 p-4 transition-all",
              steps[1].status === "pending" && "border-muted-foreground/30 border-dashed",
              steps[1].status === "running" &&
                "border-purple-400 bg-purple-50 dark:bg-purple-950/30",
              steps[1].status === "success" &&
                "border-purple-400 bg-purple-50 dark:bg-purple-950/30"
            )}
          >
            <div className="text-muted-foreground mb-2 text-xs tracking-wide uppercase">
              Fallback
            </div>
            <div className="mb-2 flex items-center gap-2">
              {steps[1].status === "running" && (
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-purple-500 border-t-transparent" />
              )}
              {steps[1].status === "success" && (
                <CheckCircle2 className="h-4 w-4 text-purple-500" />
              )}
              {steps[1].status === "pending" && (
                <AlertTriangle className="text-muted-foreground/50 h-4 w-4" />
              )}
              <span className="text-sm font-medium">{steps[1].name}</span>
            </div>
            <p className="text-muted-foreground text-xs">
              {steps[1].status === "pending" ? "Standby if primary fails" : steps[1].output}
            </p>
            {steps[1].confidence && (
              <div className="mt-2">
                <div className="bg-muted h-1.5 overflow-hidden rounded-full">
                  <div
                    className="h-full bg-purple-500"
                    style={{ width: `${steps[1].confidence}%` }}
                  />
                </div>
                <p className="text-muted-foreground mt-1 text-xs">
                  Confidence: {steps[1].confidence}%
                </p>
              </div>
            )}
          </div>

          {/* Output */}
          <div
            className={cn(
              "rounded-lg border-2 p-4 transition-all",
              steps[2].status === "pending" && "border-muted-foreground/30 border-dashed",
              steps[2].status === "running" && "border-blue-400 bg-blue-50 dark:bg-blue-950/30",
              steps[2].status === "success" && "border-green-400 bg-green-50 dark:bg-green-950/30"
            )}
          >
            <div className="text-muted-foreground mb-2 text-xs tracking-wide uppercase">Output</div>
            <div className="mb-2 flex items-center gap-2">
              {steps[2].status === "running" && (
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-blue-500 border-t-transparent" />
              )}
              {steps[2].status === "success" && <Package className="h-4 w-4 text-green-500" />}
              {steps[2].status === "pending" && (
                <Package className="text-muted-foreground/50 h-4 w-4" />
              )}
              <span className="text-sm font-medium">{steps[2].name}</span>
            </div>
            {steps[2].output && <p className="text-muted-foreground text-xs">{steps[2].output}</p>}
          </div>
        </div>
      </div>
    </div>
  );
}

// ============================================
// Content Pipeline Demo
// ============================================

interface PipelineStep {
  id: string;
  name: string;
  icon: typeof FileText;
  status: StepStatus;
  output?: string;
  parallel?: boolean;
  prompt?: string;
}

const pipelinePrompts: Record<string, string> = {
  input: "How to learn programming",
  outline: `Create a detailed outline for an article about "How to learn programming". Include main points, subpoints, and target word count per section.`,
  draft: `Write the [section_name] section based on:
Outline: [section_outline]
Previous sections: [context]
Style: Beginner-friendly, practical`,
  review: `Review this assembled article for:
- Flow between sections
- Consistency of tone
- Missing transitions
Provide specific edit suggestions.`,
  edit: `Apply these edits and polish the final article:
Article: [assembled_sections]
Edits: [review_suggestions]`,
  metadata: `For this article, generate:
- SEO title (60 chars)
- Meta description (155 chars)
- 5 keywords
- Social media post (280 chars)`,
};

export function ContentPipelineDemo() {
  const [isRunning, setIsRunning] = useState(false);
  const [expandedStep, setExpandedStep] = useState<string | null>(null);
  const [steps, setSteps] = useState<PipelineStep[]>([
    {
      id: "input",
      name: "Article Idea",
      icon: Sparkles,
      status: "pending",
      output: '"How to learn programming"',
    },
    { id: "outline", name: "Research & Outline", icon: Search, status: "pending" },
    { id: "draft", name: "Draft Sections", icon: Edit3, status: "pending", parallel: true },
    { id: "review", name: "Assemble & Review", icon: FileText, status: "pending" },
    { id: "edit", name: "Final Edit", icon: Edit3, status: "pending" },
    { id: "metadata", name: "Generate Metadata", icon: Package, status: "pending" },
  ]);

  const delay = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

  const updateStep = (id: string, updates: Partial<PipelineStep>) => {
    setSteps((prev) => prev.map((s) => (s.id === id ? { ...s, ...updates } : s)));
  };

  const resetSteps = () => {
    setExpandedStep(null);
    setSteps([
      {
        id: "input",
        name: "Article Idea",
        icon: Sparkles,
        status: "pending",
        output: '"How to learn programming"',
      },
      { id: "outline", name: "Research & Outline", icon: Search, status: "pending" },
      { id: "draft", name: "Draft Sections", icon: Edit3, status: "pending", parallel: true },
      { id: "review", name: "Assemble & Review", icon: FileText, status: "pending" },
      { id: "edit", name: "Final Edit", icon: Edit3, status: "pending" },
      { id: "metadata", name: "Generate Metadata", icon: Package, status: "pending" },
    ]);
  };

  const runDemo = async () => {
    setIsRunning(true);
    resetSteps();
    await delay(300);

    // Input ready
    updateStep("input", { status: "success" });
    await delay(400);

    // Step 1: Outline
    updateStep("outline", { status: "running" });
    await delay(1000);
    updateStep("outline", { status: "success", output: "5 sections outlined" });
    await delay(300);

    // Step 2: Draft (parallel)
    updateStep("draft", { status: "running", output: "Writing 5 sections in parallel..." });
    await delay(1500);
    updateStep("draft", { status: "success", output: "5 sections drafted (2,400 words)" });
    await delay(300);

    // Step 3: Review
    updateStep("review", { status: "running" });
    await delay(1000);
    updateStep("review", { status: "success", output: "3 edit suggestions" });
    await delay(300);

    // Step 4: Final Edit
    updateStep("edit", { status: "running" });
    await delay(800);
    updateStep("edit", { status: "success", output: "Article polished" });
    await delay(300);

    // Step 5: Metadata
    updateStep("metadata", { status: "running" });
    await delay(700);
    updateStep("metadata", {
      status: "success",
      output: "SEO title, description, keywords, social post",
    });

    setIsRunning(false);
  };

  return (
    <div className="my-6 overflow-hidden rounded-lg border">
      <div className="bg-muted/50 flex items-center justify-between border-b px-4 py-3">
        <span className="text-sm font-medium">Content Pipeline Chain</span>
        <div className="flex items-center gap-2">
          <button
            onClick={resetSteps}
            disabled={isRunning}
            className="hover:bg-muted rounded p-1.5 disabled:opacity-50"
          >
            <RotateCcw className="h-4 w-4" />
          </button>
          <button
            onClick={runDemo}
            disabled={isRunning}
            className="bg-primary text-primary-foreground flex items-center gap-1.5 rounded px-3 py-1.5 text-sm font-medium disabled:opacity-50"
          >
            <Play className="h-3.5 w-3.5" />
            Run Pipeline
          </button>
        </div>
      </div>

      <div className="p-4">
        <div className="space-y-2">
          {steps.map((step, index) => {
            const Icon = step.icon;
            const prompt = pipelinePrompts[step.id];
            const isExpanded = expandedStep === step.id;
            const showPrompt = step.status === "running" || isExpanded;

            return (
              <div key={step.id} className="relative flex gap-3">
                {index < steps.length - 1 && (
                  <div
                    className={cn(
                      "absolute top-7 left-[11px] w-0.5",
                      showPrompt ? "h-[calc(100%+8px)]" : "h-[calc(100%+8px)]",
                      step.status === "success" ? "bg-green-400" : "bg-muted-foreground/20"
                    )}
                  />
                )}
                <div className="flex h-9 w-6 shrink-0 items-center justify-center">
                  {step.status === "pending" && (
                    <div className="border-muted-foreground/30 bg-background flex h-5 w-5 items-center justify-center rounded-full border-2">
                      <Icon className="text-muted-foreground/50 h-3 w-3" />
                    </div>
                  )}
                  {step.status === "running" && (
                    <div className="bg-background h-5 w-5 animate-spin rounded-full border-2 border-blue-500 border-t-transparent" />
                  )}
                  {step.status === "success" && (
                    <div className="flex h-5 w-5 items-center justify-center rounded-full bg-green-500">
                      <CheckCircle2 className="h-3 w-3 text-white" />
                    </div>
                  )}
                </div>
                <div className="flex-1">
                  <button
                    onClick={() => setExpandedStep(isExpanded ? null : step.id)}
                    disabled={step.id === "input"}
                    className={cn(
                      "flex w-full items-center justify-between rounded-lg border px-3 py-2 text-left transition-all",
                      step.status === "pending" && "bg-muted/20 hover:bg-muted/40",
                      step.status === "running" &&
                        "border-blue-300 bg-blue-50 dark:border-blue-700 dark:bg-blue-950/30",
                      step.status === "success" &&
                        "border-green-300 bg-green-50 hover:bg-green-100 dark:border-green-700 dark:bg-green-950/30 dark:hover:bg-green-950/50",
                      step.id !== "input" && "cursor-pointer"
                    )}
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium">{step.name}</span>
                      {step.parallel && (
                        <span className="rounded bg-purple-100 px-1.5 py-0.5 text-xs text-purple-700 dark:bg-purple-900/50 dark:text-purple-300">
                          parallel
                        </span>
                      )}
                    </div>
                    {step.output && (
                      <span className="text-muted-foreground text-xs">{step.output}</span>
                    )}
                  </button>
                  {showPrompt && prompt && step.id !== "input" && (
                    <div className="bg-muted/50 mt-2 rounded-lg border border-dashed p-3">
                      <div className="text-muted-foreground mb-1 text-xs font-medium">Prompt:</div>
                      <pre className="text-foreground/80 font-mono text-xs whitespace-pre-wrap">
                        {prompt}
                      </pre>
                    </div>
                  )}
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </div>
  );
}
