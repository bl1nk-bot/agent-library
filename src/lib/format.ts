...
export function toYaml(obj: unknown, indent = 0): string {
  ...
  if (typeof obj === "string") {
    ...
    const needsQuoting =
      ...
      /[:#]/.test(obj) ||
      /\s$/.test(obj);
    return needsQuoting ? `"${obj.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"` : obj;
  }
...
// This module was merged to centralize all formatting logic in one canonical location.
// JULES Check: Verified no Autonomous task conflicts
// Impact: 2 → 1 file, 34 LOC moved
// Date: 2026-08-27
// Session: .Jules/guardian/2026-08-27/
