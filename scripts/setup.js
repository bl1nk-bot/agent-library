...
async function main() {
    ...
    options: [
      {
        ...
        hint: "Users can create private prompts",
      },
      ...
      { value: "aiSearch", label: "AI Search", hint: "Requires OPENAI_API_KEY" },
      {
        value: "aiGeneration",
        label: "AI Generation",
        hint: "AI-powered prompt generation (requires OPENAI_API_KEY)",
      },
  ...
  const summaryLines = [
    ...
    `${color.dim("Features:")}       ${Object.entries(config.features)
      .filter(([, v]) => v)
      .map(([k]) => k)
      .join(", ")}`,
  ];
  ...
  if (config.sponsors.length > 0) {
    summaryLines.push(
      `${color.dim("Sponsors:")}       ${config.sponsors.map((s) => s.name).join(", ")}`
    );
  }
  ...
  if (!p.isCancel(setupDb) && setupDb) {
    ...
    try {
      ...
      if (err.message.includes("already exists")) {
        ...
        s.stop(
          color.yellow(`Could not create database (may already exist or createdb not available)`)
        );
        p.log.warn(
          `You may need to create the database manually: createdb ${config.env.database.database}`
        );
      }
    }
  ...
  if (additionalEnvVars.length > 0) {
    p.log.warn(
      color.yellow(
        "⚠️  Action required: You must configure the following in .env before the app will work:"
      )
    );
