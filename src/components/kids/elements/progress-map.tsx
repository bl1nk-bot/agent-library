...
export function ProgressMap() {
  ...
  return (
    <div className="relative flex h-full flex-col">
      ...
      <div className="absolute top-1/2 left-2 z-20 hidden -translate-y-1/2 md:flex">
        <button
          onClick={scrollLeft}
      ...
      <div className="absolute top-1/2 right-2 z-20 hidden -translate-y-1/2 md:flex">
        <button
          onClick={scrollRight}
          className="flex h-10 w-10 items-center justify-center border-3 border-[#DAA520] bg-[#FFD700] shadow-lg transition-colors hover:bg-[#FFC000]"
          style={{
      ...
      <div
        ref={scrollRef}
        className="scrollbar-hide flex flex-1 justify-center overflow-x-auto overflow-y-hidden"
      ...
      >
        ...
        >
          ...
          <div
            className="absolute right-0 bottom-0 left-0 h-12 bg-[#8B5A2B]"
            style={{ minWidth: `${mapWidth}px` }}
          />
          <div
            className="absolute right-0 bottom-12 left-0 h-4 bg-[#228B22]"
            style={{ minWidth: `${mapWidth}px` }}
          />
          <div
            className="absolute right-0 bottom-16 left-0 h-2 bg-[#32CD32]"
            style={{ minWidth: `${mapWidth}px` }}
          />

          {/* Road with dashed center line */}
          <div
            className="absolute right-0 bottom-0 left-0 h-8 bg-[#4A4A4A]"
            style={{ minWidth: `${mapWidth}px` }}
          >
            {/* Road edges */}
            <div className="absolute top-0 right-0 left-0 h-1 bg-[#6B6B6B]" />
            <div className="absolute right-0 bottom-0 left-0 h-1 bg-[#3A3A3A]" />
            {/* Dashed center line */}
            <div
              className="absolute top-1/2 right-0 left-0 h-1.5 -translate-y-1/2 bg-repeat-x"
              style={{
                backgroundImage:
                  "repeating-linear-gradient(90deg, #FFD700 0px, #FFD700 20px, transparent 20px, transparent 40px)",
          ...
          {worlds.map((world) => {
            ...
            return (
              ...
              >
                <div
                  className="border-3 border-[#DAA520] bg-[#FFD700] px-4 py-2 text-base font-bold whitespace-nowrap text-[#8B4513]"
                  style={{
                    clipPath:
      ...
      <div className="shrink-0 border-t-4 border-[#8B4513] bg-[#2C1810] px-4 py-3">
        <div className="mx-auto max-w-md">
          <div className="mb-2 flex items-center justify-between text-xl">
            <span className="text-2xl font-bold text-[#FFD700]">{t("map.title")}</span>
          ...
          <div className="relative h-6 overflow-hidden border-2 border-[#8B4513] bg-[#4A3728]">
            <div
            ...
            <div className="absolute inset-0 flex">
              {Array.from({ length: allLevels.length }).map((_, i) => (
                <div key={i} className="flex-1 border-r border-[#2C1810] last:border-r-0" />
              ))}
            </div>
          </div>
        </div>
  ...
  for (let i = 0; i < Math.floor(mapWidth / spacing); i++) {
    ...
    decorations.push(
      ...
      >
        ...
        {type === 3 && <PixelTree className="h-14 w-10" />}
    ...
    >
      ...
      >
        ...
        <div className="flex h-10 w-4 flex-col items-center justify-center gap-0.5 bg-gray-700 p-1">
          ...
          <div className="h-2 w-2 bg-yellow-800" />
          <div className="h-2 w-2 bg-green-800" />
        </div>
        {/* Pole */}
        <div className="h-8 w-1.5 bg-gray-500" />
      </div>
...
function LevelNode({ level, position, progress, index, t }: LevelNodeProps) {
  ...
  const nodeContent = (
    <div
      className={cn(
        "absolute -translate-x-1/2 -translate-y-1/2 transform transition-all duration-300",
    ...
    >
      ...
      <PixelLevelNode state={state} levelNumber={`${level.world}.${level.levelNumber}`} />

      {/* Stars for completed levels */}
      {isCompleted && (
        <div className="absolute -bottom-1 left-1/2 flex -translate-x-1/2 gap-0">
          {[1, 2, 3].map((star) => (
      ...
      <div
        className="absolute top-full left-1/2 mt-6 -translate-x-1/2 border-2 border-amber-400 bg-amber-100 px-3 py-2 text-base font-bold whitespace-nowrap dark:bg-amber-900"
        style={{
          clipPath:
            "polygon(4px 0%, calc(100% - 4px) 0%, 100% 4px, 100% calc(100% - 4px), calc(100% - 4px) 100%, 4px 100%, 0% calc(100% - 4px), 0% 4px)",
        }}
