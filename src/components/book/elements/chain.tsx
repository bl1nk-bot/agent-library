...
const chainColors: Record<
  string,
  { bg: string; border: string; text: string; stepBg: string; arrow: string }
> = {
  blue: {
    bg: "bg-blue-50 dark:bg-blue-950/30",
    border: "border-blue-200 dark:border-blue-800",
    text: "text-blue-700 dark:text-blue-300",
    stepBg: "bg-blue-100 dark:bg-blue-900/50",
    arrow: "text-blue-400",
  },
  purple: {
    ...
    stepBg: "bg-purple-100 dark:bg-purple-900/50",
  ...
  amber: {
    ...
    stepBg: "bg-amber-100 dark:bg-amber-900/50",
  ...
  green: {
    ...
    stepBg: "bg-green-100 dark:bg-green-900/50",
  ...
  return (
    <div className="my-4 overflow-hidden rounded-lg border">
      <div className="bg-muted/30 flex items-center justify-between border-b px-4 py-2">
        <span className="text-muted-foreground text-xs capitalize">{type} chain</span>
        <button
                ...
                <div key={i}>
                  <div className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div
                        className={cn(
                          "flex h-6 w-6 items-center justify-center rounded-full border text-xs font-medium transition-all duration-300",
                    ...
                    <div
                      className={cn(
                        ...
                        isSkipped && isStepActive(i)
                          ? "opacity-50"
                          : isStepActive(i) || isStepLoading(i)
                            ? "opacity-100"
                            : "opacity-50"
                      )}
                    >
                      <p
                        className={cn(
                          "m-0! mb-2 text-sm font-medium",
                          isSkipped && isStepActive(i) ? "text-muted-foreground line-through" : ""
                        )}
                      >
                        {step.step}
                      </p>
                      {!isSkipped && (
                        <>
                          <div className="bg-muted/50 mb-2 rounded border p-3 text-sm">
                            <p className="text-muted-foreground m-0! mb-1 text-xs">Prompt:</p>
                            <p className="m-0! font-mono text-xs whitespace-pre-wrap">
                              {step.prompt}
                            </p>
                          </div>
                          <div
                            className={cn(
                              "rounded border p-3 text-sm transition-colors duration-300",
                              isStepActive(i)
                                ? "border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/30"
                                : "bg-background"
                            )}
                          >
                            <p className="text-muted-foreground m-0! mb-1 text-xs">Output:</p>
                            <p className="m-0! font-mono text-xs whitespace-pre-wrap">
                              {step.output}
                            </p>
                          </div>
          ...
          : steps.map((step, i) => (
              <div key={i}>
                <div className="flex gap-3">
                  ...
                  >
                    <p className="m-0! mb-2 text-sm font-medium">{step.step}</p>
                    <div className="bg-muted/50 mb-2 rounded border p-3 text-sm">
                      <p className="text-muted-foreground m-0! mb-1 text-xs">Prompt:</p>
        ...
        {type === "iterative" && (
          <div className="mt-3 space-y-2">
            ...
            {iterationOutput && currentIteration === 2 && (
              <div className="mx-4 rounded border border-dashed border-amber-300 bg-amber-50/50 p-2 dark:border-amber-700 dark:bg-amber-950/20">
  ...
  return (
    <div className="my-6 overflow-hidden rounded-lg border">
      ...
      <div className={cn("p-6", colors.bg)}>
        <p className={cn("m-0! mb-4! text-sm font-medium", colors.text)}>{selected.description}</p>
        ...
        <div className="flex items-center justify-center gap-2">
          {selected.id === "parallel" ? (
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="flex gap-2">
                {selected.steps.map((step, i) => (
                  ...
                  >
                    <p className={cn("m-0! text-xs font-medium", colors.text)}>{step.label}</p>
                    <p className="text-muted-foreground m-0! text-xs">{step.desc}</p>
                  </div>
                ))}
              </div>
              <ChevronDown className={cn("h-4 w-4", colors.arrow)} />
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              >
                <p className={cn("m-0! text-xs font-medium", colors.text)}>
                  {selected.steps[0].label}
                </p>
                <p className="text-muted-foreground m-0! text-xs">{selected.steps[0].desc}</p>
              </div>
              <div className="flex gap-8">
                <div className="flex flex-col items-center">
                  ...
                  >
                    <p className={cn("m-0! text-xs font-medium", colors.text)}>
                      {selected.steps[1].label}
                    </p>
                    <p className="text-muted-foreground m-0! text-xs">{selected.steps[1].desc}</p>
                  </div>
                </div>
                <div className="flex flex-col items-center">
                  <ChevronDown className={cn("h-4 w-4", colors.arrow)} />
                  ...
                  >
                    <p className={cn("m-0! text-xs font-medium", colors.text)}>
                      {selected.steps[2].label}
                    </p>
                    <p className="text-muted-foreground m-0! text-xs">{selected.steps[2].desc}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              {selected.steps.map((step, i) => (
                <Fragment key={i}>
                  <div
                    className={cn(
                      "rounded-lg px-4 py-2 text-center transition-all duration-300",
                      colors.stepBg,
                      activeStep === i ? cn("ring-2", colors.border, "scale-105") : "opacity-70"
                    )}
                  >
