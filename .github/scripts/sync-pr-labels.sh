#!/usr/bin/env bash
#
# Sync labels from linked issue to pull request.
# - Copies all labels from the issue that a PR closes
# - Removes labels on the PR that are not on the issue (cleanup)
# - Does not require an issue – independent PRs are allowed
#
# Usage: ./sync-pr-labels.sh [PR_NUMBER]
#   If PR_NUMBER is omitted, processes all open PRs in the repository.
#

set -euo pipefail

# Initialize a comma-separated string to hold PR numbers that need a comment (not used now)
PRS_NEEDING_COMMENT=""
